...
sed -i "5c\@ IN SOA $domaindns. root.$domaindns. (" $dirb
sed -i "12c\@ IN NS $domaindns." $dirb
sed -i "13c\@ IN A $ipdns1.$ipdns2.$ipdns3.$ipdns4" $dirb
sed -i "14c\ $subdomaindns IN A $ipdns1.$ipdns2.$ipdns3.$ipdns4" $dirb
...
sed -i "5c\@ IN SOA $domaindns. root.$domaindns. (" $dirc
sed -i "12c\@ IN NS $domaindns." $dirc
...
dirnet="/etc/network/interfaces"
echo "auto eth0:0" >> $dirnet
echo "iface eth0:0 inet static" >> $dirnet
echo "        address 192.168.2.2" >> $dirnet
echo "        netmask 255.255.255.0" >> $dirnet
service networking restart
dbf="/etc/bind/db.forward"
sed -i "14c\mail IN A 192.168.2.2" $dbf
echo "@ IN AAAA ::1" >> $dbf
dbr="/etc/bind/db.reverse"
echo "2 IN PTR mail.bintang.com." >> $dbr
service bind9 restart
dirsq="/etc/squirrelmail/apache.conf"
echo "<VirtualHost 192.168.2.2:80>" >> $dirsq
echo " ServerName mail.bintang.com" >> $dirsq
echo " DocumentRoot /usr/share/squirrelmail" >> $dirsq
echo "</VirtualHost>" >> $dirsq
echo "Include /etc/squirrelmail/apache.conf" >> /etc/apache2/apache2.conf
service apache2 restart
clear
echo "Please Run adduser manually"
read ok
banner
}
